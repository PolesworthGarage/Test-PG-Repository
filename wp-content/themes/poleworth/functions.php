...
if( !function_exists( 'AMJ_MainDebug' ) ) {
	function AMJ_MainDebug($sValue){
		...
		if ($m_fAMJ_MainDebug){
			...
			file_put_contents( $UploadPath . "/AMJ_SearchDebug.txt", date("F j, Y, g:i") . " AMJ_MainDebug, $sValue\n", FILE_APPEND );
		}
	}
}
	...
	function the_result( $id ) {
		...
		echo '<li data-price="' . strtolower( $fields['Price'][0] ) . '" data-year="' . $fields['Year'][0] . '" data-doors="' . strtolower( $fields['Doors'][0] ) . '" data-body="' . strtolower(  $fields['Bodytype'][0] ) . '" data-fuel="' . strtolower(  $fields['FuelType'][0] ) . '" data-size="' . $fields['EngineSize'][0] . '">';
			...
			if( $post_timestamp >= $three_days ) {
				...
				echo '<img src="' . get_bloginfo( 'stylesheet_directory' ) .'/images/new-arrival.png" id="new-arrivals-overlay" style="position:absolute; width: 75px; height: 75px; border: none" />';
				...
				if( $fields['Price'][0] == '' ) {
					...
					if( in_array( $fields['FullRegistration'][0], $registrations ) ) {
						...
						if( !empty( $fields['_finance_quote'][0] ) && $fields['_finance_quote'][0] != '0' && $fields['_finance_quote'][0] != '0.00' ) {
							...
							$sFinanceQuote = (string)$fields['_finance_quote'][0];
							
							echo '<p class="finance-quote">£' . $sFinanceQuote . ' month</p>';
				...
				echo '</div>';
						...
						echo '<ul class="main-details alignleft">';
							if( $fields['Bodytype'][0] != '' ):
								echo '<li>' . ((string)$fields['Bodytype'][0]) . ',</li>';
							endif;
							if( $fields['Year'][0] != '' ):
								echo '<li>' . ((string)$fields['Year'][0]) . ',</li>';
							endif;
							if( $fields['Mileage'][0] != '' ):
								echo '<li>' . ((string)$fields['Mileage'][0]) . ' Miles,</li>';
							endif;
							if( $fields['FuelType'][0] != '' ):
							echo '<li> ' . ((string)$fields['FuelType'][0]) . ',</li>';
							endif;
							if( $fields['Transmission'][0] != '' ):
								echo '<li>' . ((string)$fields['Transmission'][0]) . '</li>';
							endif;
						echo '</ul>';
...
if( !function_exists( 'attach_pages' ) ) {
	function attach_car_table(  ) {
		...
		add_menu_page( 'Nearly New', 'Nearly New', 'manage_options', 'pg_finance', 'pg_finance', 'dashicons-admin-tools', '58.6' ); 		
	}
...
if( !function_exists( 'pg_finance' ) ) {
	...
	function pg_finance(  ) {
		...
		$registrations = get_option( 'pg_finance_registrations1', array(  ) );
		...
		if( strtoupper( $_SERVER['REQUEST_METHOD'] ) === 'POST' ):
			...
			if( $_POST['action'] == 'finance' ):
				...
				update_option( 'pg_finance_registrations1', $registrations );
		...
		<div class="wrap">
			...
			<h2>Nearly New 17 Plate Cars</h2>
	...
	function trade_form(  ) {
		if( isset( $_POST['trade-submit'] ) ) {
			if( empty( $_POST['trade-name'] ) || empty( $_POST['trade-email'] ) ) {
				...
				include_once 'includes/MCAPI.php';
				$api_key = 'e6c91c77521ab198c33f4d9fdc256d0f-us14';
				$trade_list = '0e85df4908';
				$api = new MCAPI($api_key);
			...
			echo '<form method="post">';
				echo '<input type="text" name="trade-name" placeholder="Your Name" style="width: 208px; clear: both;" /><br />';
				echo '<input type="text" name="trade-email" placeholder="Your Email" style="width: 208px; clear: both;" /><br />';
				echo '<input type="submit" id="trade-submit" name="trade-submit" value="Submit" style="width: 208px; margin-top: 10px; clear: both;" />';
			echo '</form>';
