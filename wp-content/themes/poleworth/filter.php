...
?>
<?php
	        if (strpos($_SERVER['REQUEST_URI'], "search/cars") >= 1) {
  $refine_class = "hideme";
}
else {
  $refine_class = "showme";
	...
	<form action="<?php bloginfo('home'); ?>/search/cars" method="post" class="car-search" id="new-search" style="margin-top: 0px!important;">
		...
		<table width="100%">
			<tr>
				...
				<td><label for="min "class="minimum-label">Min &pound;</label></td>
				<td></td>
				<td><label for="max" class="maximum-label">Max &pound;</label></td>
				<td></td>
			</tr>
			<tr>
				<td>
	...
	?>" style="display:none">
		...
		<div class="filter-box">
			...
			<div class="range">
				...
				<label>
					<input type="text" class="max" name="max" <?php if( $refine['max'] &&  $refine['max'] < 100000 ) { echo 'value="' . $refine['max'] . '"'; } ?> />
					<div>Max £</div>
				</label>
			</div>
		</div>
